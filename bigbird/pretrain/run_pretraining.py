...
class PrintLossHook(tf.estimator.SessionRunHook):
  ...
  def __init__(self,
    ...
    self._timer = (
        tf.compat.v1.train.NeverTriggerTimer() if only_log_at_end else tf.compat.v1.train.SecondOrStepTimer(
  ...
  def _log_tensors(self, tensor_values):
    ...
    if self._formatter:
      ...
      if self._every_n_iter is not None and elapsed_secs is not None:
        logging.warn("%s speed: (%.3f step/s)", ", ".join(stats), self._every_n_iter / elapsed_secs)
      else:
        logging.warn("%s", ", ".join(stats))
...
def model_fn_builder(bert_config):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      #logging_hook = tf.compat.v1.train.LoggingTensorHook({"loss" : total_loss}, every_n_iter=1)
      logging_hook = PrintLossHook({"global_step":global_step, "loss" : total_loss})
